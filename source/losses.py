...
class SCELoss(torch.nn.Module):
    ...
    def forward(self, pred, labels):
        ...
        # Softmax prediction (detach so it doesn't keep backward graph)
        pred_soft = F.softmax(pred, dim=1).detach()
        pred_soft = torch.clamp(pred_soft, min=1e-7, max=1.0)

        # One-hot encode labels (no grad required)
        label_one_hot = F.one_hot(labels, self.num_classes).float()
        label_one_hot = torch.clamp(label_one_hot, min=1e-4, max=1.0)
        label_one_hot = label_one_hot.detach()

        # Reverse Cross-Entropy
        rce = -torch.sum(pred_soft * torch.log(label_one_hot), dim=1).mean()
